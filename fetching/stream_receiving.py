...
import fetching.formulator
import fetching.log as log
from fetching.data_models import TsSegment

from detection.object_tracker import detect
...
class M3u8Watcher:
    ...
    class Config:
        ...
        formulator: fetching.formulator.M3u8Formulator = None  # Класс наследние от M3u8Formulator, используемый для
    ...
    def __close_file__(self):
        log.n(self.tag, "uTS закрыт: " + self.__cur_ts_file_name__ + " ("
              ...
              + " сек)")
        detect({
            video: '../data_storage/uTS/' + self.__cur_ts_file_name__,
            output: '../data_storage_output/uTS/' + self.__cur_ts_file_name__,
        })
